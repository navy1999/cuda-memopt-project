...
__global__ void matmul_tiled_vec(float *A, float *B, float *C, int n) {
    __shared__ vec4 As[TILE_SIZE][TILE_SIZE/4];
    __shared__ vec4 Bs[TILE_SIZE][TILE_SIZE/4];
    ...
    for(int t=0;t<iterations;++t){
        ...
        for(int k=0;k<TILE_SIZE/4;++k){
            vec4 a = As[threadIdx.y][k];
            vec4 b = Bs[k][threadIdx.x/4];
...
int main(int argc, char **argv) {
    ...
    if (N % 4 != 0) { printf("Vectorized kernel requires N divisible by 4.\n"); return 1; }
    const int trials=10; size_t bytes=(size_t)N*N*sizeof(float);
    ...
    for(int t=0;t<trials;++t){
        ...
        matmul_tiled_vec<<<grid,block>>>(d_A,d_B,d_C,N);
